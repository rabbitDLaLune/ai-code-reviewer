...
export default function App() {
  ...
  useEffect(() => {
    const savedHistory = getReviewHistory();
    setHistory(savedHistory);
    setReviewCount(savedHistory.length);
  }, []);

  const formatDetailedReview = (reviewData: ReviewResult) => {
    return `Detected Language: ${reviewData.detectedLanguage || "Unknown"}
...
${
  reviewData.issues && reviewData.issues.length > 0
    ? reviewData.issues.map((issue) => `• ${issue}`).join("\n")
...
${
  ...
  const handleReview = async () => {
    ...
    try {
      ...
      const result: ReviewResult = {
        ...data.review,
        rawReview:
          data.review.rawReview || formatDetailedReview(data.review),
      };
      ...
      setReviewCount(updatedHistory.length);
  ...
  return (
    <main
      className={`min-h-screen overflow-hidden px-6 py-8 transition-colors duration-300 ${
        isDark
    ...
    >
      <div
        className={`pointer-events-none fixed -left-48 -top-48 h-[600px] w-[600px] rounded-full blur-3xl ${
          isDark ? "bg-violet-600/10" : "bg-violet-300/30"
        }`}
      />

      <div
        className={`pointer-events-none fixed -bottom-48 -right-48 h-[700px] w-[700px] rounded-full blur-3xl ${
          isDark ? "bg-cyan-400/10" : "bg-cyan-300/30"
        }`}
      />
      ...
      <div className="relative z-10 mx-auto max-w-7xl">
        ...
        <div
          className={isDark ? "mb-8 h-px bg-white/10" : "mb-8 h-px bg-slate-200"}
        />
